...
@Service
@AllArgsConstructor
public class OkHttpSendTextMessageClient {
	
	private final SendMessageToWhatsApp sendMessageToWhatsApp;
	
	public JSONObject sendMessage(String messagingProduct,String recipientPhoneNumber,String previousMessageId,boolean previewURL,String textBody,String version,String phoneNumberID, String token) throws Exception
	{
		...
		try {
			...
			textObject.put(SEND_MESSAGE_KEYS.preview_url.name(), previewURL);
			textObject.put(SEND_MESSAGE_KEYS.body.name(), textBody);
			...
			bodyObject.put(SEND_MESSAGE_KEYS.type.name(), MESSAGE_TYPE.text.name());
			bodyObject.put(SEND_MESSAGE_KEYS.text.name(), textObject.toString());
