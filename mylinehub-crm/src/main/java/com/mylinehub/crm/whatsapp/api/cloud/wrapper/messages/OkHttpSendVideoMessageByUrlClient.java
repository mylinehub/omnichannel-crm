...
@Service
@AllArgsConstructor
public class OkHttpSendVideoMessageByUrlClient {

	private final SendMessageToWhatsApp sendMessageToWhatsApp;
	
	public JSONObject sendMessage(String caption,String fileName,String messagingProduct,String recipientPhoneNumber,String previousMessageId,String videoCaption,String videoUrl,String version,String phoneNumberID, String token) throws Exception
	{
		...
		try {
			...
			videoObject.put(SEND_MESSAGE_KEYS.link.name(), videoUrl);
			if(caption!=null)
				videoObject.put(SEND_MESSAGE_KEYS.caption.name(), caption);
			if(fileName!=null)
				videoObject.put(SEND_MESSAGE_KEYS.filename.name(), fileName);
			...
			bodyObject.put(SEND_MESSAGE_KEYS.type.name(), MESSAGE_TYPE.video.name());
			bodyObject.put(SEND_MESSAGE_KEYS.video.name(), videoObject.toString());
