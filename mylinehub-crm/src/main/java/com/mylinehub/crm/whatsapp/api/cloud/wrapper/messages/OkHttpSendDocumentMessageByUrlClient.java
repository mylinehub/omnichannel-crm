...
@Service
@AllArgsConstructor
public class OkHttpSendDocumentMessageByUrlClient {

	private final SendMessageToWhatsApp sendMessageToWhatsApp;
	
	public JSONObject sendMessage(String caption,String fileName,String messagingProduct,String recipientPhoneNumber,String previousMessageId,String documentUrl,String version,String phoneNumberID, String token) throws Exception
	{
		...
		try {
			...
			documentObject.put(SEND_MESSAGE_KEYS.link.name(), documentUrl);
			if(caption!=null)
				documentObject.put(SEND_MESSAGE_KEYS.caption.name(), caption);
			if(fileName!=null)
				documentObject.put(SEND_MESSAGE_KEYS.filename.name(), fileName);
			...
			bodyObject.put(SEND_MESSAGE_KEYS.type.name(), MESSAGE_TYPE.document.name());
			bodyObject.put(SEND_MESSAGE_KEYS.document.name(), documentObject.toString());
