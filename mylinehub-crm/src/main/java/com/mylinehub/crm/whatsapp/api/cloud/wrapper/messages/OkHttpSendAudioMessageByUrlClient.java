...
@Service
@AllArgsConstructor
public class OkHttpSendAudioMessageByUrlClient {

    private final SendMessageToWhatsApp sendMessageToWhatsApp;
	
    public JSONObject sendMessage(String caption,String filename,String messagingProduct,String recipientPhoneNumber,String previousMessageId,String audioUrl,String version,String phoneNumberID, String token) throws Exception
	{
		...
		try {
			...
			audioObject.put(SEND_MESSAGE_KEYS.link.name(), audioUrl);
			if(caption!= null)
				audioObject.put(SEND_MESSAGE_KEYS.caption.name(), caption);
			if(filename!= null)
				audioObject.put(SEND_MESSAGE_KEYS.filename.name(), filename);
			...
			bodyObject.put(SEND_MESSAGE_KEYS.type.name(), MESSAGE_TYPE.audio.name());
			bodyObject.put(SEND_MESSAGE_KEYS.audio.name(), audioObject.toString());
