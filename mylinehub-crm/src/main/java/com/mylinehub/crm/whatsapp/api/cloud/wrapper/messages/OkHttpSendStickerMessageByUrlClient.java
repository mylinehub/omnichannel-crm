...
@Service
@AllArgsConstructor
public class OkHttpSendStickerMessageByUrlClient {

	private final SendMessageToWhatsApp sendMessageToWhatsApp;
	
	public JSONObject sendMessage(String caption,String fileName, String messagingProduct,String recipientPhoneNumber,String previousMessageId,String stickerUrl,String version,String phoneNumberID, String token) throws Exception
	{
		...
		try {
			...
			stickerObject.put(SEND_MESSAGE_KEYS.link.name(), stickerUrl);
			if(caption!=null)
				stickerObject.put(SEND_MESSAGE_KEYS.caption.name(), caption);
			if(fileName!=null)
				stickerObject.put(SEND_MESSAGE_KEYS.filename.name(), fileName);
			...
			bodyObject.put(SEND_MESSAGE_KEYS.type.name(), MESSAGE_TYPE.sticker.name());
			bodyObject.put(SEND_MESSAGE_KEYS.sticker.name(), stickerObject.toString());
