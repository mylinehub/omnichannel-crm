 ...
 */
@AllArgsConstructor
public class ExportAMIConnectionToXLSX implements ExcelColumnsHeaderWriter {
    ...
    private final List<AmiConnection> amiconnections;
    ...
    private void writeCellsData(Sheet sheet) {
        int rowNum = 1;
        for (AmiConnection amiconnection : amiconnections) {
            Row row = sheet.createRow(rowNum++);
            row.createCell(0).setCellValue(amiconnection.getId());
            row.createCell(1).setCellValue(amiconnection.getPhonecontext());
            row.createCell(2).setCellValue(amiconnection.getOrganization());
            row.createCell(3).setCellValue(amiconnection.getAmiuser());
            row.createCell(4).setCellValue(amiconnection.getPassword());
            row.createCell(5).setCellValue(amiconnection.getPort());  
            row.createCell(6).setCellValue(amiconnection.getDomain());
            row.createCell(7).setCellValue(amiconnection.isIsactive());
    ...
    public void export(HttpServletResponse response) throws IOException {
        ...
        String headerValue = "amiconnections_" + currentDateTime + ".xlsx";
