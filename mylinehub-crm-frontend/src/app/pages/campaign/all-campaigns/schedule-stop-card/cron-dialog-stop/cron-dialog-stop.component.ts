...
@Component({
  ...
  templateUrl: './cron-dialog-stop.component.html',
  styleUrls: ['./cron-dialog-stop.component.scss']
})
export class CronDialogStopComponent  implements OnInit, OnDestroy {
  ...
  constructor(protected ref: NbDialogRef<CronDialogStopComponent>,
  ...
  submit() {
   ...
   // console.log("cron : "+ submitCron);
    ...
    this.campaignService.scheduleCronStopCampaign({campaignId : JSON.parse(JSON.stringify(this.currentRecord)).id,cronExpression:submitCron,organization:this.organization,fromExtension:JSON.parse(JSON.stringify(this.currentRecord)).extension,domain:JSON.parse(JSON.stringify(this.currentRecord)).domain,data:"data",actionType:"actionType"})
