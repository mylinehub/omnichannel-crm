  ...
  async buttonClicked()
  {
   // console.log("actionButtonClicked");
    ...
    if(this.title == 'Enable/Disable')
    {
      if(this.currentRecord.active)
      {
        this.sshConnectionService.disableSshConnectionOnIdAndOrganization( {id: this.currentRecord.id,organization: this.organization})
      ...
      {
        this.sshConnectionService.enableSshConnectionOnIdAndOrganization( {id: this.currentRecord.id,organization: this.organization})
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
            if(String(result) == 'true')
            {
             // console.log("Result is true, setting values to show on icon");
              this.on = true;
              this.show = 'ON';
              this.currentRecord.active = true;
              this.parent.emit('Enable/Disable');
            }
            else{
              //  console.log("Result is not true");
    ...
    else if(this.title == 'Refresh')
    {
      ...
      this.sshConnectionService.refreshSshConnectionForOrganization(this.currentRecord.domain,this.organization)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
            if(String(result) == 'true')
