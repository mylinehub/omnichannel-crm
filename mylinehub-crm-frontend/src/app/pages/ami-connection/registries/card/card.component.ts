  ...
  async buttonClicked()
  {
   // console.log("actionButtonClicked");
    ...
    if(this.title == 'Enable/Disable')
    {
      if(this.currentRecord.isactive)
      {
        this.amiConnectionService.disableAmiConnectionOnAmiUserAndOrganization( {amiuser: this.currentRecord.amiuser,organization: this.organization})
      ...
      {
        this.amiConnectionService.enableAmiConnectionOnAmiUserAndOrganization( {amiuser: this.currentRecord.amiuser,organization: this.organization})
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
            if(String(result) == 'true')
    ...
    else if(this.title == 'Refresh')
    {
      this.on = false;
      this.show = 'Refreshing';
      
      this.amiConnectionService.refreshAmiConnectionForOrganization(this.currentRecord.domain,this.organization)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
            if(String(result) == 'true')
    ...
    else if(this.title == 'Connect')
    {
      this.on = false;
      this.show = 'Connecting';

      this.amiConnectionService.connectAmiConnectionOnAmiUserAndOrganization({amiuser: this.currentRecord.amiuser,organization: this.organization})
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
            if(String(result) == 'true')
            {
              ...
              this.show = 'Connected';
                ...
                //console.log("Result is not true");
                this.showDialoge('Error','activity-outline','danger', "Connection was not successful"); 
                this.on = true;
