...
export class AllPurchasesComponent implements OnInit, OnDestroy {
  ...
  allDropDownOptions = [
    { column: 1, label: 'Purchase Date Less', properties:['purchaseDate']},
    { column: 1, label: 'Purchase Date Greater', properties:['purchaseDate']},
    { column: 1, label: 'Customer ID', properties:['customerId']},
  ];
...
setTableDropDownValues()
       ...
       // console.log("getAllEmployeesByOrganization");
        ...
        if(allData == null)
        {
          ...
          arr.forEach((element) => {
            ...
            singleObject.value = element.name;
            singleObject.title = element.name;
            list.push( JSON.parse(JSON.stringify(singleObject)));
          });
        }

       
        this.settings.columns.purchaseName.editor.config.list = [... list];
  ...
  setTable()
  {
    this.purchaseService.getAllPurchasesOnOrganization(this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: allData => {
  ...
  onDeleteConfirm(event): void {
   ...
   this.purchaseService.deletePurchaseByIdAndOrganization(this.currentRecord.id,this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: result => {
  ...
  onSaveConfirm(event): void {
    ...
    if(this.previousRecord.customerId !== this.currentRecord.customerId){
      var id = this.currentRecord.customerId.split(',')[0];
    }
    else{
      console.log('Both customerId ID was same.')
    }
    //console.log("Id : " + id);

    var arr = JSON.parse(JSON.stringify(this.allCustomers));
    arr.forEach((element) => {
            //console.log(JSON.stringify(element));
            if(element.id == id)
            {
              //console.log("Inside Create loop and found manager");
              this.currentRecord.customerId = id;
              this.currentRecord.customerFirstName = element.firstname;
              this.currentRecord.customerLastName = element.lastname;
            }
          });
    ...
    if (window.confirm('Are you sure you want to edit?')) {
      ...
      this.purchaseService.updatePurchaseByOrganization(JSON.stringify(this.currentRecord),this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: result => {
  ...
  onCreateConfirm(event): void {
    ...
    if (window.confirm('Are you sure you want to create?')) {
      ...
      this.purchaseService.createPurchaseByOrganization(JSON.stringify(this.currentRecord),this.organization)
       .pipe(takeUntil(this.destroy$))
       .subscribe({
         next: result => {
  ...
  if(this.currentDropDownOption.label == 'Purchase Date Less')
    {
      ...
      this.purchaseService.findAllByPurchaseDateLessThanEqualAndOrganization(result,this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
    ...
    else if(this.currentDropDownOption.label == 'Purchase Date Greater')
    {
      //console.log("Phone Context Search Started");
      ...
      this.purchaseService.findAllByPurchaseDateGreaterThanEqualAndOrganization(result,this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
  ...
  else if(this.currentDropDownOption.label == 'Customer ID')
  {
    //console.log("Phone Context Search Started");
    this.purchaseService.findAllByCustomerAndOrganization(this.inputValues[0],this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: allData => {
  ...
  this.exportAsConfig.type = this.downloadAs;
    this.exportAsService
      .save(this.exportAsConfig, 'Purchases')
      .subscribe(() => {
        // save started
      });
  }
