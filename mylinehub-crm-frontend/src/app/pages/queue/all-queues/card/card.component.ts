  ...
  async buttonClicked()
  {
   // console.log("actionButtonClicked");
    ...
    if(this.title == 'Enable/Disable')
    {
      if(this.currentRecord.isactive)
      {
        this.queueService.disableQueueOnEmailAndOrganization( {extension: this.currentRecord.extension,organization: this.organization})
      ...
      {
        this.queueService.enableQueueOnExtensionAndOrganization( {extension: this.currentRecord.extension,organization: this.organization})
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
            if(String(result) == 'true')
