...
export class AllQueuesComponent implements OnInit, OnDestroy {
  
  tableHeading = 'All Queues';
  ...
  onFilechange(event: any) {
   // console.log("onFilechange");
    ...
    if(event.target.files[0].name.endsWith(".xlsm") || event.target.files[0].name.endsWith(".xlsx") || event.target.files[0].name.endsWith(".xls")) 
    {
      ...
      this.queueService.upload(formParams,this.organization)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
  ...
  onDeleteConfirm(event): void {
   ...
   this.queueService.deleteQueueByExtensionAndOrganization(this.currentRecord.extension,this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: result => {
  ...
  onSaveConfirm(event): void {
    ...
    if (window.confirm('Are you sure you want to edit?')) {
     ...
     // console.log("Starting Update API");
      this.queueService.updateQueueByOrganization(JSON.stringify(this.currentRecord),this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: result => {
  ...
  onCreateConfirm(event): void {
    ...
    if (window.confirm('Are you sure you want to create?')) {
      ...
      this.queueService.createQueueByOrganization(JSON.stringify(this.currentRecord),this.organization)
       .pipe(takeUntil(this.destroy$))
       .subscribe({
         next: result => {
  ...
  onSearch()
  {
   ...
   //console.log("On Search is clicked");
    ...
    // console.log("Ami User Search Started");
      this.queueService.getQueueByExtensionAndOrganization(this.inputValues[0],this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
    ...
    else if(this.currentDropDownOption.label == 'PhoneContext')
    {
      ...
      this.queueService.getAllQueuesOnPhoneContextAndOrganization(this.inputValues[0],this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
    ...
    else if(this.currentDropDownOption.label == 'IsActive')
    {
      ...
      this.queueService.getAllQueueOnIsEnabledAndOrganization(this.inputValues[0],this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
  ...
  export() {
    ...
    this.exportAsService
      .save(this.exportAsConfig, 'Queue')
      .subscribe(() => {
        // save started
      });
  }
