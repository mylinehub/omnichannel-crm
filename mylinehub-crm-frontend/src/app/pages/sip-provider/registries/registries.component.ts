...
EmployeeService
  ...
  allDropDownOptions = [
    { column: 1, label: 'PhoneNumber', properties:['phoneNumber']},
  ];
 ...
 exportAsConfig: ExportAsConfig = {
  ...
  constructor(private sipProviderService : SipProviderService,
  ...
  setTableDropDownValues()
  {
    ...
    .subscribe({
      next: allData => {
        //console.log("getAllCostCalcultationType");
    ...
    .subscribe({
      next: allData => {
        //console.log("getAllMeteredPlanAmount");
        //console.log(JSON.stringify(allData));
        var list= [];
        var singleObject = {value: '', title:''};
        if(allData == null)
        ...
        this.settings.columns.meteredPlanAmount.editor.config.list = [... list];
        this.settings = Object.assign({}, this.settings);
  ...
  setTable()
  {
    this.sipProviderService.getAllSipProvidersByOrganization(this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: allData => {
  ...
  onDeleteConfirm(event): void {
   ...
   this.sipProviderService.deleteSipProviderByPhoneNumberAndOrganization(this.currentRecord.phoneNumber,this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: result => {
  ...
  onSaveConfirm(event): void {
    ...
    if (window.confirm('Are you sure you want to edit?')) {
     ...
     // console.log("Starting Update API");
      this.sipProviderService.updateSipProviderByOrganization(JSON.stringify(this.currentRecord),this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: result => {
  ...
  onCreateConfirm(event): void {
    ...
    if (window.confirm('Are you sure you want to create?')) {
      ...
      this.sipProviderService.createSipProviderByOrganization(JSON.stringify(this.currentRecord),this.organization)
       .pipe(takeUntil(this.destroy$))
       .subscribe({
         next: result => {
  ...
  onSearch()
  {
   ...
   //console.log("On Search is clicked");
    ...
    if(this.currentDropDownOption.label == 'PhoneNumber')
    {
    // console.log("Ami User Search Started");
      this.sipProviderService.getSipProviderByPhoneNumberAndOrganization(this.inputValues[0],this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
  ...
  this.exportAsConfig.type = this.downloadAs;
    this.exportAsService
      .save(this.exportAsConfig, 'SipProvider')
      .subscribe(() => {
        // save started
      });
  }
