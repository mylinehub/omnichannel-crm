 ...
 ngOnInit(): void {
  ...
  async buttonClicked()
  {
    ...
    if(this.title == 'Enable/Disable')
    {
      if(this.currentRecord.active)
      {
        this.sipProviderService.disableSipProviderOnIdAndOrganization( {id: this.currentRecord.id,organization: this.organization})
        ...
        this.sipProviderService.enableSipProviderOnIdAndOrganization( {id: this.currentRecord.id,organization: this.organization})
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
            if(String(result) == 'true')
