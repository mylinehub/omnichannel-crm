...
@Component({
  selector: 'ngx-all-conferences',
  templateUrl: './all-conferences.component.html',
  styleUrls: ['./all-conferences.component.scss']
})
export class AllConferencesComponent implements OnInit, OnDestroy {
  
  tableHeading = 'All Conferences';
  ...
  allDropDownOptions = [
    { column: 1, label: 'Extension', properties:['confextension']},
    { column: 1, label: 'PhoneContext', properties:['phonecontext']},
    { column: 1, label: 'IsActive', properties:['isconferenceactive']},
  ];
        ...
        else
        {
            //console.log("Inside Data : "+ JSON.stringify(allData));
              ...
              arr.forEach((element) => {
                ...
                if (element.isconferenceactive == true) {
  ...
  onFilechange(event: any) {
   // console.log("onFilechange");
    ...
    if(event.target.files[0].name.endsWith(".xlsm") || event.target.files[0].name.endsWith(".xlsx") || event.target.files[0].name.endsWith(".xls")) 
    {
      ...
      this.conferenceService.upload(formParams,this.organization)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
  ...
  onDeleteConfirm(event): void {
   ...
   this.conferenceService.deleteConferenceByExtensionAndOrganization(this.currentRecord.confextension,this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: result => {
        ...
        if(String(result) == 'true')
        {
             ...
             //console.log("this.currentRecord.isconferenceactive");
             //console.log(JSON.stringify(this.currentRecord.isconferenceactive));


              if(JSON.stringify(this.currentRecord.isconferenceactive) == "\"true\""||JSON.stringify(this.currentRecord.isconferenceactive) == "true"){
  ...
  onSaveConfirm(event): void {
    ...
    if (window.confirm('Are you sure you want to edit?')) {
     ...
     // console.log("Starting Update API");
      this.conferenceService.updateConferenceByOrganization(JSON.stringify(this.currentRecord),this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: result => {
        ...
        if(String(result) == 'true')
        {
            // console.log("Result is true");
             ...
             if(JSON.stringify(this.currentRecord.isconferenceactive) == JSON.stringify(this.previousRecord.isconferenceactive)){   
              ...
              else if(JSON.stringify(this.currentRecord.isconferenceactive) == "\"false\""||JSON.stringify(this.currentRecord.isconferenceactive) == "false")
                {
  ...
  onCreateConfirm(event): void {
    ...
    if (window.confirm('Are you sure you want to create?')) {
      ...
      this.conferenceService.createConferenceByOrganization(JSON.stringify(this.currentRecord),this.organization)
       .pipe(takeUntil(this.destroy$))
       .subscribe({
         next: result => {
  ...
  onSearch()
  {
   ...
   //console.log("On Search is clicked");
    ...
    // console.log("Ami User Search Started");
      this.conferenceService.getConferenceByExtensionAndOrganization(this.inputValues[0],this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
    ...
    else if(this.currentDropDownOption.label == 'PhoneContext')
    {
      ...
      this.conferenceService.getAllConferenceOnPhoneContextAndOrganization(this.inputValues[0],this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
    ...
    else if(this.currentDropDownOption.label == 'IsActive')
    {
      ...
      this.conferenceService.getAllConferenceOnIsEnabledAndOrganization(this.inputValues[0],this.organization)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: allData => {
    ...
    if(event == 'Enable/Disable')
    {
      ...
      //console.log(JSON.stringify(this.allRecords));
        ...
        Object.keys(this.currentRecord).forEach((element) => { //console.log(key);
          if(element == "isconferenceactive")
          {
            this.values[i] = this.currentRecord.isconferenceactive;
          }
          i = i +1;
        });
  ...
  this.exportAsConfig.type = this.downloadAs;
    this.exportAsService
      .save(this.exportAsConfig, 'conference')
      .subscribe(() => {
        // save started
      });
  }
