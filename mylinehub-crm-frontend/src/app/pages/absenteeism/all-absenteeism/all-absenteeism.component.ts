...
@Component({
  selector: 'ngx-all-absenteeism',
  templateUrl: './all-absenteeism.component.html',
  styleUrls: ['./all-absenteeism.component.scss']
})
export class AllAbsenteeismComponent implements OnInit, OnDestroy {
  
  tableHeading = 'All Absenteeism';
  ...
  allDropDownOptions = [
    { column: 1, label: 'Employee ID', properties:['employeeID']},
    { column: 1, label: 'Reason For Absenteeism', properties:['reasonForAbsense']},
    { column: 2, label: 'B/W From,To Date', properties:['dateFrom','dateTo']},
  ];
  ...
  setTable()
  {
    this.absenteeismService.getAllAbsenteeismOnOrganization(this.organization)
    .pipe(takeUntil(this.destroy$))
    .subscribe({
      next: allData => {
  ...
  onSearch()
  {
   ...
   if(this.currentDropDownOption.label == 'Employee ID')
   {
  ...
  export() {
    ...
    this.exportAsService
      .save(this.exportAsConfig, 'AllAbsenteeism')
      .subscribe(() => {
        // save started
      });
  }
