  ...
  async buttonClicked()
  {
   // console.log("actionButtonClicked");
    ...
    if(this.title == 'Enable/Disable')
    {
      if(this.currentRecord.isactive)
      {
        this.ivrService.disableIvrOnEmailAndOrganization( {extension: this.currentRecord.extension,organization: this.organization})
      ...
      {
        this.ivrService.enableIvrOnExtensionAndOrganization( {extension: this.currentRecord.extension,organization: this.organization})
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: result => {
            if(String(result) == 'true')
